<template>
  <section class="relative py-12 px-4">
    <div class="bg-secondary rounded-3xl max-w-screen-xl mx-auto px-12 py-12 flex flex-col md:flex-row items-stretch justify-between gap-8 relative h-full">
      <div class="md:w-5/12 z-0 flex flex-col justify-between h-full">
        <div class="flex-grow md:h-[330px]">
          <h3 class="text-sm text-primary tracking-widest uppercase font-semibold">Blog</h3>
          <h2 class="text-3xl font-bold text-primary mt-2">Güncel Yazılarla Yanınızdayız</h2>
          <p class="text-gray-500 mt-2 max-w-md">
            Çocuk gelişimi, hijyen, pratik öneriler ve daha fazlası blog sayfamızda.
          </p>
          <div class="mt-6">
            <button class="bg-primary text-white px-5 py-3 rounded hover:bg-primary-dark transition text-sm">
              Tümünü Gör →
            </button>
          </div>
        </div>
        ...
        >
          <SwiperSlide v-for="(blog, i) in blogs" :key="i" class="pr-4 pb-1">
            <div class="rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-transform transform hover:scale-[1.01] bg-white">
              <img :src="blog.image" :alt="blog.title" class="w-full h-[300px] object-cover" />
              <div class="p-4">
                <h3 class="text-lg font-semibold text-gray-900 leading-tight">{{ blog.title }}</h3>
