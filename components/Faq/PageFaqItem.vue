<template>
  <div class="py-6 border-b border-gray-200 last:border-none">
    <button class="flex justify-between items-start w-full text-left" @click="open = !open">
      ...
      <Icon :name="open ? 'mdi:close' : 'mdi:plus'"
        class="w-5 h-5 text-primary shrink-0 mt-1 transition-transform duration-200" />
    </button>

    <Transition name="fade">
      <div v-show="open" class="pl-10 pr-2 mt-3 text-sm text-gray-600 leading-relaxed">
        {{ answer }}
      </div>
    </Transition>
  </div>
